...
#include <cstdlib>
  ...
  Input::Input()
  {
    ...
    int Input::validateInput()
    {
      ...
      { cout << "\nERROR: Please select an appropriate visualization method"; status = 1; }
      if (getoutput() > 3 || getoutput() < 0 || (getoutput() == 0 && output[0] != '0'))
      { cout << "\nERROR: Please select an appropriate output type"; status = 1; }
      if (getformat() > 1 || getformat() < 0 || (getformat() == 0 && format[0] != '0'))
      { cout << "\nERROR: Please select an appropriate still image output format"; status = 1; }
      if (getvisMethod() == 'F' && getoutput() == 1 && getformat() == 1)
      { cout << "\nERROR: EPS output cannot be used with the Fog vizualization method"; status = 1; }
      if (getperiodic() == 1 && getoutput() == 1 && getformat() == 1)
      { cout << "\nERROR: EPS output cannot be used with periodic plotting"; status = 1; }
      if (getimageW() < 0 || (getimageW() == 0 && imageW[0] != '0'))
      { cout << "\nERROR: Please enter a positive integer or '0' for image width"; status = 1; }
      if (getimageH() < 0 || (getimageH() == 0 && imageH[0] != '0'))
      { cout << "\nERROR: Please enter a positive integer or '0' for image height"; status = 1; }
      if (getframerate() < 1)
      { cout << "\nERROR: Please enter a positive value >=1 for frame rate"; status = 1; }
      if (getduration() < 0.1)
      { cout << "\nERROR: Please enter a positive value >=1 for animation duration"; status = 1; }
      if (getstart_r() < 0 || (getstart_r() == 0 && start_r[0] != '0'))
      { cout << "\nERROR: Please enter a positive value for start 'r'"; status = 1; }
      if (getstart_phi() < -3600 || getstart_phi() > 3600 || (getstart_phi() == 0 && start_phi[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for start 'phi'"; status = 1; }
      if (getstart_theta() < -3600 || getstart_theta() > 3600 || (getstart_theta() == 0 && start_theta[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for start 'theta'"; status = 1; }
      if (getend_r() < 0 || (getend_r() == 0 && end_r[0] != '0'))
      { cout << "\nERROR: Please enter a positive value for end 'r'"; status = 1; }
      if (getend_phi() < -3600 || getend_phi() > 3600 || (getend_phi() == 0 && end_phi[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for end 'phi'"; status = 1; }
      if (getend_theta() < -3600 || getend_theta() > 3600 || (getend_theta() == 0 && end_theta[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for end 'theta'"; status = 1; }
      if (getfly() > 1 || getfly() < 0 || (getfly() == 0 && fly[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for fly-through"; status = 1; }
      if (getflip() > 1 || getflip() < 0 || (getflip() == 0 && flip[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for flip on fly-through"; status = 1; }
      if (getelev_there() < -3600 || getelev_there() > 3600 || (getelev_there() == 0 && elev_there[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for elevation of rotation towards end point"; status = 1; }
      if (getfull() > 1 || getfull() < 0 || (getfull() == 0 && full[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for full rotation"; status = 1; }
      if (getelev_back() < -3600 || getelev_back() > 3600 || (getelev_back() == 0 && elev_back[0] != '0'))
      { cout << "\nERROR: Please enter a numeric value for elevation of rotation back from end point"; status = 1; }
      if (getx_tran() == 0 && x_tran[0] != '0')
      { cout << "\nERROR: Please enter a numeric value for x translation"; status = 1; }
      if (gety_tran() == 0 && y_tran[0] != '0')
      { cout << "\nERROR: Please enter a numeric value for y translation"; status = 1; }
      if (getz_tran() == 0 && z_tran[0] != '0')
      { cout << "\nERROR: Please enter a numeric value for z translation"; status = 1; }
      if (getbg_b() > 1 || getbg_b() < 0 || (getbg_b() == 0 && bg_b[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the blue component of the background"; status = 1; }
      if (getbg_g() > 1 || getbg_g() < 0 || (getbg_g() == 0 && bg_g[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the green component of the background"; status = 1; }
      if (getbg_r() > 1 || getbg_r() < 0 || (getbg_r() == 0 && bg_r[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the red component of the background"; status = 1; }
      if (gettext_b() > 1 || gettext_b() < 0 || (gettext_b() == 0 && text_b[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the blue component of the text"; status = 1; }
      if (gettext() > 2 || gettext() < 0 || (gettext() == 0 && text[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for on-image text"; status = 1; }
      if (gettext_g() > 1 || gettext_g() < 0 || (gettext_g() == 0 && text_g[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the green component of the textd"; status = 1; }
      if (gettext_r() > 1 || gettext_r() < 0 || (gettext_r() == 0 && text_r[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the red component of the text"; status = 1; }
      if (getthreshold() > 1 || getthreshold() < 0 || (getthreshold() == 0 && threshold[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the probability threshold"; status = 1; }
      if (getcolourMethod() > 1 || getcolourMethod() < 0 || (getcolourMethod() == 0 && colourMethod[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for colouring method"; status = 1; }
      if (getthreshold() > 1 || getthreshold() < 0 || (getthreshold() == 0 && threshold[0] != '0'))
      { cout << "\nERROR: Please enter a value between 0 and 1 for the probability threshold"; status = 1; }
      if (getcolourMethod() > 1 || getcolourMethod() < 0 || (getcolourMethod() == 0 && colourMethod[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for colouring method"; status = 1; }
      if (getantiA() > 15 || getantiA() < 1)
      { cout << "\nERROR: Please select an anti-aliasing level between 1 and 15 (larger than 4 is not recommended for large data sets)"; status = 1; }
      if (getperiodic() > 1 || getperiodic() < 0 || (getperiodic() == 0 && periodic[0] != '0'))
      { cout << "\nERROR: Please select either '0' or '1' for periodic plotting"; status = 1; }
      if (getperiodic() == 1 && getvisMethod() == 'F')
      { cout << "\nERROR: Please select either cube or bubble visualisation methods for periodic plotting"; status = 1; }
